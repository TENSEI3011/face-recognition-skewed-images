...
run_pose_study.py
-----------------
WHAT : Pose-stratified evaluation — measures how Rank-1 accuracy degrades
       as yaw (left-right head turn), pitch (up-down tilt), and altitude increase.
WHY  : UAV cameras view faces from oblique angles. This experiment reveals WHICH
       angle is most damaging to recognition and at what threshold accuracy becomes
       unusable. Results justify the hybrid pipeline's inclusion of geometry and
       HOG features that are more pose-robust than ArcFace alone.

       APPROACH: Probe images are synthetically generated by applying geometric
       transforms (rotation for yaw, perspective warp for pitch, downsampling for
       altitude) to gallery/probe images via synthesize_pose_probes.py.
       The gallery is always clean; only probes have pose applied.
